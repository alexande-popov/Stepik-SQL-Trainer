...
[Нумерация строк с помощью переменных](#Нумерация-строк-с-помощью-переменных)
...
## Нумерация строк с помощью переменных
...
INNER JOIN 
  (
  SELECT program_id, enrollee_id,
         IF(@num_pr = program_id, 
            @row_num := @row_num + 1, 
            @row_num := 1 AND @num_pr := program_id) AS str_num
  FROM applicant_order
  ) AS a USING(program_id, enrollee_id)
SET str_id = str_num;
```
```
+--------+------------+-------------+------+
